...
class YnabSpider(Spider):
  ...
  def parse(self, response):
    page_count = int(response.xpath(
      '//div[@id="pageNav"]/a[5]/text()').extract_first())

    page_urls = ['https://support.youneedabudget.com/topics?pg={}&sort=no_sort'
      .format(x) for x in range(1,page_count)]
  ...
  def parse_posts_list(self, response):
    post_urls = response.xpath(
      '//a[@class="topic-summary__text-link -topic"]/@href').extract()

    for url in ['https://support.youneedabudget.com{}'
      .format(x) for x in post_urls]:
  ...
  def parse_post_page(self, response):
    ...
    user = response.xpath(
      '//li[@class="topic-meta__item topic-meta__handle at-handle"]/text()'
    ).extract_first()
    
    posted = "" # NEED FROM SELENIUM SCRIPT
    
    category = response.xpath(
      '//a[@class="site-breadcrumb__link -category"][2]/text()'
    ).extract_first()
    
    replies = response.xpath(
      '//span[@class="panel__value replyCount"]/text()'
    ).extract_first()
    
    views = response.xpath(
      '//div[@class="panel panel-stats"]/ul/li[4]/span[1]/text()'
    ).extract_first()
    
    last_active = "" # NEED FROM SELENIUM SCRIPT
    
    text = response.xpath(
      '//div[@class="cfa topic__text formatted"]/p/text()'
    ).extract()
